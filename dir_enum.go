...
func EnumerateDIR(alvo string, wordlist string, threads int) (ScanReport, error) {
	...
	if err != nil {
		// Ao invés de printar, retornamos o erro para quem chamou a função lidar com ele
		return report, fmt.Errorf("erro ao abrir a wordlist: %v", err)
	}
	defer file.Close()

	// Opcional: Você pode manter esses prints iniciais se quiser feedback no terminal,
	// ou removê-los se quiser que a função seja 100% silenciosa.
	fmt.Printf("[*] Iniciando enumeração silenciosa em: %s\n", alvo)

	var wg sync.WaitGroup
	var mu sync.Mutex
	var count int
	var encontrados []DirResult // Lista onde vamos guardar os achados
	...
	for i := 1; i <= threads; i++ {
		...
		go func(id int) {
			...
			for path := range tarefas {
				...
				// Se encontrou algo, não printa, apenas salva na nossa estrutura!
